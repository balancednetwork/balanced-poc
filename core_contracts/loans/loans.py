...
class Loans(IconScoreBase):
    ...
    def get_total_collateral(self) -> int:
        sicx = self.create_interface_score(self._sICX_address.get(), TokenInterface)
        return self.get_sicx_value(sicx.balanceOf(self.address))
    ...
    def get_account_positions(self, _owner: Address) -> dict:
        ...
        for asset in self._asset_list:
            amount = self._positions[self._day_index.get()][str(self.msg.sender)][asset]
    ...
    def _asset_burn(self, _amount: int, _symbol: str) -> None:
        ...
        try:
            ...
            revert(f'Trouble burning {_symbol} tokens from {token_address}. Exception: {e}')
    ...
    def _repay_loan(self, _from: Address, _value: int) -> None:
        ...
        self._asset_burn(repaid, symbol)
        self.LoanRepaid(_from, symbol, repaid,
            f'Loan of {repaid / DENOMINATOR} {symbol} repaid to Balanced.')
    ...
    def _retire_asset(self, _from: Address, _value: int, _for: str = '') -> None:
        ...
        token = self.create_interface_score(self.msg.sender, TokenInterface)
        symbol = token.symbol()
        sicx_rate = self.get_sicx_value(DENOMINATOR)
        ...
        AssetRedeemed(_to, symbol, redeemed,
            f'{redeemed // DENOMINATOR} {symbol} redeemed on Balanced.')
    ...
    def originate_loan(self, _asset: str, _amount: int) -> None:
        ...
        new_loan_value = self.get_price_in_icx(_asset) * _amount // DENOMINATOR
        if self.get_total_position_value() + new_loan_value > max_loan_value:
            revert(f'{collateral / DENOMINATOR} collateral is insufficient '
                   f'to originate a loan of {_amount / DENOMINATOR} {_asset}'
                   f' when max_loan_value = {max_loan_value / DENOMINATOR} and new_loan_value '
                   f'= {new_loan_value / DENOMINATOR}.')
        ...
        self.OriginateLoan(self.tx.origin, _asset, _amount,
            f'Loan of {_amount / DENOMINATOR} {_asset} from Balanced.')
    ...
    def withdraw_collateral(self, _value: int) -> None:
        ...
        address = str(self.tx.origin)
        remaining_sicx = self._positions[self._day_index.get()][address]['sICX'] - _value
        ...
        if self.get_sicx_value(remaining_sicx) < locking_value:
            revert(f'Requested withdrawal is more than available collateral. '
                   ...
                   f'remaining_sicx: {remaining_sicx} '
                   f'locking_value: {locking_value}')
        self._positions[self._day_index.get()][address]['sICX'] -= _value
    ...
    def get_total_position_value(self) -> int:
        ...
        asset_value = 0
        for asset in self._asset_list:
            amount = self._positions[self._day_index.get()][str(self.tx.origin)][asset]
            if amount > 0:
                asset_value += self.get_price_in_icx(asset) * amount // DENOMINATOR
        return asset_value
    ...
    def get_sicx_value(self, _amount: int) -> int:
        ...
        value = _amount * staking.get_rate() // DENOMINATOR
        return value
